...
}function submitIssueForm(A){var B=$j(A).closest("form");
B.find(".loading").removeClass("hidden");
B.find(":submit").prop("disabled",true);
$j.ajax({type:"POST",url:baseUrl+"/issue/do_action",data:B.serialize()}).success(function(E){var F=B.closest("[data-issue-key]");
var C=F.attr("data-issue-key");
var D=$j(E);
F.replaceWith(D);
notifyIssueChange(C)
}).fail(function(C,D){closeIssueForm(A);
alert(D)
});
return false
